...
export const CreateInvoice = ({ editId, onComplete }) => {
  ...
  const loadInitialData = async () => {
    try {
      const [customersData, productsData, bankAccountsData, nextNumber] = await Promise.all([
        ...
        editId ? null : window.electronAPI.getNextDocumentNumber('INVOICE')
      ]);
      ...
      if (editId) {
        ...
        // Reset states for new invoice creation
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      alert(t('messages.invoiceSavedAsDraft'));
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      alert(t('messages.invoiceCreated', { docNumber: result.doc_number }));
      ...
      alert(t('messages.errorFinalizingInvoice'));
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      alert(t('messages.invoiceUpdated'));
      ...
      alert(t('messages.errorUpdatingInvoice'));
    }
  };
  ...
  const handleSaveDraftWhenEditing = async () => {
    ...
    try {
      ...
      await window.electronAPI.updateDocument(editId, documentData, items);
      alert(t('messages.invoiceUpdatedAsDraft'));
  ...
  const handleFinalizeWhenEditing = async () => {
    ...
    try {
      const documentData = {
        ...
        issue_date: formData.issue_date,
        tax_event_date: formData.tax_event_date,
        status: formData.status, // Keep existing status or use form data
        vat_rate: formData.vat_rate,
        exemption_reason: formData.vat_rate === 0 ? formData.exemption_reason : null,
        correction_reason: null,
        total_net: formData.total_net,
      ...
      alert(t('messages.invoiceFinalized', { docNumber: result.doc_number }));
  ...
  const handleSubmit = async (e) => {
    ...
    if (editId) {
      ...
      // For new invoices, finalize by default
